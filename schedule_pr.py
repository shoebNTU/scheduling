...
#                     cm = sns.light_palette("green", as_cmap=True, reverse=True) #
                    ...
                    df['duration-months'] = df['End Date'].dt.to_period('M').astype(int) - df['Start Date'].dt.to_period('M').astype(int) # ((df['End Date'] - df['Start Date'])/np.timedelta64(1, 'M')).astype(int)
